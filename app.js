...
(() => {
  ...
  let QUESTIONS = [
    { q: "Humans use only 10% of their brains.", options: ["Fact","Myth"], correctIndex: 1, explain: "Myth. Imaging shows we use virtually all parts of the brain." },
    { q: "Drinking water helps prevent some kidney stones.", options: ["Fact","Myth"], correctIndex: 0, explain: "Fact. Hydration lowers risk for common stone types." },
    { q: "Caffeine makes drinks 'not count' toward hydration.", options: ["Fact","Myth"], correctIndex: 1, explain: "Myth. Caffeine is mildly diuretic but fluids still count." }
  ];
  ...
  let nextQTimer = null; let ACTIVE = [];

  // Poll state
  let pollActive = false, pollEndAt = 0, pollTicker = null;
  let votesByUser = new Map(); let voteCounts = [];
  ...
  // --- Helpers ---
  const setScore = (n)=>{ state.score = Math.max(0,n); scoreVal.textContent = state.score; };
  ...
  const setBossHP = (pct)=> {
    ...
    if (state.bossHP<=0){
      ...
      gremlin.style.opacity='.6'; gremlin.style.filter='grayscale(1) contrast(.9)';
    } else {
      bossHpFill.style.filter=''; gremlin.style.opacity='1'; gremlin.style.filter='';
    }
  };
  ...
  const hideBanner = ()=>{ banner.hidden = true; banner.className = "banner"; };
  const showBanner = (text, type)=>{ banner.textContent = text; banner.className = "banner " + type; banner.hidden = false; };
  const clearTimer = ()=>{ if(nextQTimer){ clearTimeout(nextQTimer); nextQTimer = null; } };
  const setButtonsDisabled = (disabled)=>{ document.querySelectorAll("#answers button").forEach(b=> b.disabled = disabled); };
  ...
  window.addEventListener('keydown',(e)=>{ if (e.key.toLowerCase()==='p') setPaused(!state.paused); });

  // Blink loop
  function blinkOnce(){ lidL.style.opacity = lidR.style.opacity = 1; setTimeout(()=>{ lidL.style.opacity = lidR.style.opacity = 0; }, 120); }
  (function blinkLoop(){ const t = 1800 + Math.random()*2000; setTimeout(()=>{ blinkOnce(); blinkLoop(); }, t); })();

  // Impact point on ellipse (SVG viewBox 220×160)
  function randomImpactPoint(){
    const cx=110, cy=85, rx=70, ry=60;
    const theta=Math.random()*2*Math.PI, r=Math.sqrt(Math.random());
  ...
  function spawnSparkles(count = 18, origin = {x:110, y:85}){
    ...
    for (let i=0;i<count;i++){
      const s=document.createElement('div'); s.className='sparkle'; hitOverlay.appendChild(s);
      const c = palette[Math.floor(Math.random()*palette.length)];
      s.style.boxShadow = `0 0 8px rgba(255,255,255,.9), 0 0 18px ${c}cc, 0 0 32px ${c}99`;

      const angle=Math.random()*2*Math.PI, dist=60+Math.random()*120;
      const tx=Math.cos(angle)*dist, ty=Math.sin(angle)*dist;
      const spin=(Math.random()*2-1)*1080, scale=.7+Math.random()*1.1, dur=520+Math.random()*420;

      s.animate([
        { transform:`translate3d(${origin.x}px,${origin.y}px,0) translate(-50%,-50%) scale(.5) rotate(0deg)`, opacity:0 },
        { transform:`translate3d(${origin.x+tx*0.25}px,${origin.y+ty*0.25}px,0) translate(-50%,-50%) scale(${scale}) rotate(${spin*0.35}deg)`, opacity:1, offset:.3 },
        { transform:`translate3d(${origin.x+tx}px,${origin.y+ty}px,0) translate(-50%,-50%) scale(${scale*.7}) rotate(${spin}deg)`, opacity:0 }
  ...
  function spawnMagicRing(origin = {x:110, y:85}, options = {}){
    ...
    for (let i = 0; i < 1 + echo; i++){
      ...
      ], { duration: dur, easing: 'cubic-bezier(.2,.9,.3,1)', fill: 'both' });
  ...
  function gremlinHit(){
    ...
    gremlin.classList.add('magic-glow'); setTimeout(()=> gremlin.classList.remove('magic-glow'), 220);

    // face -> sad briefly; pupils drop
    const smile = document.getElementById("mouth-smile");
    const sad   = document.getElementById("mouth-sad");
    if (smile && sad){ smile.style.display="none"; sad.style.display="block"; }
    const pL = document.getElementById("pupilL");
    const pR = document.getElementById("pupilR");
    if (pL && pR){ pL.setAttribute("cy", 75); pR.setAttribute("cy", 75); }
    ...
    setTimeout(()=>{
      ...
      if (state.bossHP > 0) {
        if (smile && sad){ smile.style.display="block"; sad.style.display="none"; }
        if (pL && pR){ pL.setAttribute("cy", 74); pR.setAttribute("cy", 74); }
      }
    }, 600);
  }
  ...
  function startPoll(options){
    ...
    pollBox.hidden = false; renderPollBars(options); tickPoll();
  ...
  function renderPollBars(options){
    ...
    options.forEach((label, idx)=>{
      ...
      const lbl = document.createElement('span'); lbl.className='poll-label';
      lbl.textContent = `${idx+1}. ${label}`;

      const bar = document.createElement('div'); bar.className='poll-bar';
      const fill = document.createElement('div'); fill.className='poll-fill';
      fill.style.width = pct + '%'; bar.appendChild(fill);

      const pctEl = document.createElement('span'); pctEl.className='poll-pct';
  ...
  function tryRegisterVote(user, text, options){
    ...
    if (prev === undefined) { voteCounts[idx]++; votesByUser.set(user, idx); }
    else if (prev !== idx)   { voteCounts[prev]--; voteCounts[idx]++; votesByUser.set(user, idx); }
  ...
  function renderQuestion(){
    ...
    answersWrap.innerHTML = ""; feedback.textContent = "";
    ...
    if (ENABLE_CHAT_POLL){ startPoll(item.options); }
  }
  ...
  // Dev Buttons
  document.getElementById("btnCorrect").onclick = ()=> { if (state.paused) return; const item = ACTIVE[state.questionIndex]; handleAnswer(item.correctIndex); };
  document.getElementById("btnWrong").onclick   = ()=> { if (state.paused) return; const item = ACTIVE[state.questionIndex]; const wrongIdx = item.correctIndex===0?1:0; handleAnswer(wrongIdx); };
  document.getElementById("btnWin").onclick     = ()=> { if (state.paused) return; state.locked=true; setButtonsDisabled(true); stopGhostDance(); ghostieDefeat(); showBanner("YOU WIN!", "win"); clearTimer(); nextQTimer=setTimeout(reset,1200); };
  document.getElementById("btnLose").onclick    = ()=> { if (state.paused) return; losePlayer(); };
  document.getElementById("btnReset").onclick   = reset;
  if (pauseBtn){ pauseBtn.onclick = () => setPaused(!state.paused); pauseBtn.textContent = "Pause"; }

  // StreamElements Hook (safe in CodePen)
  try {
    window.addEventListener('onEventReceived', function (obj) {
      const d = obj && obj.detail; if (!d) return;
      if (d.listener === 'message') {
        const ev = d.event || d.data || {}; const data = ev.data || ev;
        const text = (data.text || data.message || '').toString();
        const user = (data.displayName || data.nick || data.username || data.user || '').toString();
        const item = ACTIVE[state.questionIndex];
  ...
  // Fake chat harness for CodePen
  let SIM_TIMER=null;
  const FAKE_NAMES = Array.from({length: 200}, (_,i)=> `Viewer_${(i+1).toString().padStart(3,'0')}`);
  const CMDS=['!vote 1','!vote 2','!fact','!myth'];
  function fakeVoteOnce(){
    if (!pollActive || state.paused) return;
    const name=FAKE_NAMES[Math.floor(Math.random()*FAKE_NAMES.length)];
    const text=CMDS[Math.floor(Math.random()*CMDS.length)];
    window.dispatchEvent(new CustomEvent('onEventReceived', { detail:{ listener:'message', event:{ data:{ text, displayName:name } } } }));
  }
  function startFakeVotes(ratePerSec){ stopFakeVotes(); const interval=Math.max(1,Math.floor(1000/Math.max(1,ratePerSec))); SIM_TIMER=setInterval(fakeVoteOnce,interval); }
  function stopFakeVotes(){ if(SIM_TIMER){ clearInterval(SIM_TIMER); SIM_TIMER=null; } }
  (function initSim(){
    ...
    if (!startBtn) return;
    rate.addEventListener('input', ()=>{ rateVal.textContent = rate.value + '/s'; if (SIM_TIMER) startFakeVotes(parseInt(rate.value,10)); });
  ...
  function normalizeItem(raw){
    ...
    if (typeof options === 'string') {
      options = options.split(/[|,]/g).map(s => s.trim()).filter(Boolean);
    }
  ...
  async function importFromFile(mode){ // 'replace' | 'append'
    ...
    try {
      ...
      for (const raw of data){
        try { items.push(normalizeItem(raw)); ok++; }
        catch { bad++; }
      }
      if (!ok) return setImpMsg('No valid questions found.', false);

      if (mode === 'replace') QUESTIONS = items; else QUESTIONS = QUESTIONS.concat(items);
